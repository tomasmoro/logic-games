 ...
 * @property rankingPreview comparativa mundial del jugador, para pintar en la antesala el mismo
 *   panel que el diálogo de fin de partida ANTES de jugar (ver
 *   [com.kortexgames.app.domain.repository.ProgressRepository.previewRanking]). Tabla única (el
 *   juego no separa por dificultad): `null` mientras se resuelve ([rankingPreviewLoading]) o si
 *   no hay comparativa que mostrar (invitado, sin red, o sin ninguna marca todavía).
 * @property rankingPreviewLoading `true` mientras se pide [rankingPreview] tras entrar en la
 *   antesala. Arranca en `true` (no en `false`) para no enseñar el aviso de "sin comparativa" un
 *   instante antes de que llegue.
 */
